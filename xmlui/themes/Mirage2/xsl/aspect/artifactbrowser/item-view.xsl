...
<xsl:stylesheet
                ...
                <div class="col-sm-4">
                    <div class="row">
                        <div class="col-xs-6 col-sm-12">
                            <xsl:call-template name="itemSummaryView-DIM-thumbnail-download"/>
                        </div>
                         <div class="col-xs-6 col-sm-12">
        ...
        <div class="item-summary-view-metadata">
            ...
            <div class="row">
                ...
                <div class="col-sm-8">
                    ...
                    <xsl:call-template name="itemSummaryView-collections"/>
					<!-- CIATCE
					<xsl:call-template name="itemSummaryView-DIM-cite"/><br/> 
					KONEC CIATCE -->
					<xsl:call-template name="itemSummaryView-DIM-cite-test"/><br/>
					<br/>
                </div>
            </div>
        </div>
    ...
    <xsl:template name="itemSummaryView-DIM-thumbnail-download">
        <div class="thumbnail">
            <xsl:for-each select="dim:field[@element='relation' and @qualifier='uri']">
                <a title="Download article" target="_blank" href="{.}">
                   <img alt="Download article" src="{concat($theme-path, 'images/save.jpg')}" width="125" height="125"/>
                </a>
            </xsl:for-each>    
        </div>  
    </xsl:template> 
...
<!-- CITACE -->
<xsl:template name="itemSummaryView-DIM-cite">
         <xsl:if test="dim:field[@element='identifier' and @qualifier='doi']">
         <div class="simple-item-view-cite">
               
                <xsl:for-each select="dim:field[@element='identifier' and @qualifier='doi']">                     
                   
                    <script type="text/javascript" src="{concat($theme-path, 'scripts/jquery-min.js')}"></script>
                    <script type="text/javascript" src="{concat($theme-path, 'scripts/jquery-mendelu.js')}"></script>   
				<h5>
                    Citace
                </h5> 
				<a target="blank">
                    <xsl:attribute name="href">https://zbib.org/import?q=<xsl:copy-of select="./node()"/>
                    </xsl:attribute>
										
                    <xsl:copy-of select="./node()"/>
                </a>
                    <!--<SPAN class="doi-url">https://doi.org/<xsl:copy-of select="node()"/></SPAN>  -->
...
<!--VELKA CITACE-->
<xsl:template name="itemSummaryView-DIM-cite-test">
<h5>Citace
                </h5>
<!--START Citace pro formu C_KNIHA - KAPITOLA-->
				

<xsl:if test="contains(., 'C_KNIHA - KAPITOLA')">                        	
...
<xsl:if test="dim:field[@element='contributor'][@qualifier='author' and descendant::text()] or dim:field[@element='creator' and descendant::text()] or dim:field[@element='contributor' and descendant::text()]">
        ...
        <xsl:if test="dim:field[@element='date' and @qualifier='issued' and descendant::text()]">
            <xsl:for-each select="dim:field[@element='date' and @qualifier='issued']">
                    <xsl:text>. </xsl:text>
					<nobr><xsl:text></xsl:text>
					<xsl:copy-of select="substring(./node(),1,10)"/>
					<xsl:text>. </xsl:text></nobr>
                    <xsl:if test="count(following-sibling::dim:field[@element='date' and @qualifier='issued']) != 0">
                        <br/>
                    </xsl:if>
                </xsl:for-each>
			...
			<!--START title idealne jen v jedné jazykove forme, problem u cs publikaci s nezadoucim en nazvem-->
		<xsl:choose>
            <xsl:when test="count(dim:field[@element='title'][not(@qualifier)]) &gt; 1">
                    <xsl:for-each select="dim:field[@element='title'][not(@qualifier)]">
                            
                               <span style="font-style: italic"> <xsl:value-of select="./node()"/></span>
                                <xsl:if test="count(following-sibling::dim:field[@element='title'][not(@qualifier)]) != 0">
                                    <xsl:text>; (</xsl:text>
                                <nobr/>
                            </xsl:if>
        ...
        </xsl:choose><!--STOP title idealne jen v jedné jazykove forme, problem u cs publikaci s nezadoucim en nazvem-->
			
			<!--START In pred C_KNIHA - KAPITOLA-->
		<xsl:choose>
        ...
        </xsl:choose>	<!--STOP In pred C_KNIHA - KAPITOLA-->
		
		<!--ispartof - nazev knihy-->
		<xsl:choose>
		...
		<!--STOP ispartof - nazev knihy-->
		
		
			<!--START Vydavatel-->
		<xsl:choose>
            <xsl:when test="count(dim:field[@element='publisher'][not(@qualifier)]) &gt; 1">
            ...
            <xsl:when test="count(dim:field[@element='publisher'][not(@qualifier)]) = 1">
                    ...
                    <xsl:value-of select="dim:field[@element='publisher'][not(@qualifier)][1]/node()"/>
                
            </xsl:when>
            <xsl:otherwise>
                    <i18n:text> </i18n:text>
            </xsl:otherwise>
        </xsl:choose><!--STOP Vydavatel-->

		
<!--START format - strany - v kapitole rozsah stran-->
		<xsl:choose>
            <xsl:when test="dim:field[@element='format'][not(@qualifier)]">
            ...
            <xsl:when test="count(dim:field[@element='local'][not(@qualifier)]) = 1">
                
                    <xsl:value-of select="dim:field[@element='local'][not(@qualifier)][1]/node()"/>. 
                
            </xsl:when>
            <xsl:otherwise>
                    <xsl:text> </xsl:text>
            </xsl:otherwise>
        </xsl:choose><!--STOP format - strany - v kapitole rozsah stran-->
		...
		<xsl:choose>
            ...
            <xsl:when test="count(dim:field[@element='identifier'][@qualifier='isbn']) = 1">
                
                    <xsl:value-of select="dim:field[@element='identifier'][@qualifier='isbn'][1]/node()"/>. 
                
            </xsl:when>
            <xsl:otherwise>
                    <i18n:text> . </i18n:text>
            </xsl:otherwise>
        </xsl:choose><!--STOP isbn-->
...
<!--START doi-->
	...
	<!--asi není potřeba :-) <xsl:if test="dim:field[@element='identifier' and @qualifier='uri' and descendant::text()]">
        ...
        </xsl:if><!--STOP Citace pro formu C_KNIHA - KAPITOLA-->



<!--START Citace pro formu J_ČLÁNEK-->
<xsl:if test="contains(., 'J_ČLÁNEK')">
...
<!--title idealne jen v jedné jazykove forme, problem u cs publikaci s nezadoucim en nazvem-->
		<xsl:choose>
            <xsl:when test="count(dim:field[@element='title'][not(@qualifier)]) &gt; 1">
                    <xsl:for-each select="dim:field[@element='title'][not(@qualifier)]">
...
<!--volume - rocnik-->
		<xsl:choose>
            <xsl:when test="dim:field[@element='volume'][not(@qualifier)]">
			
                    <xsl:for-each select="dim:field[@element='volume'][not(@qualifier)]">
                            
                                <xsl:value-of select="./node()"/>
                                <xsl:if test="count(following-sibling::dim:field[@element='volume'][not(@qualifier)]) != 0">
                            </xsl:if>
...
<!--number - cislo-->		
		<xsl:choose>
            <xsl:when test="dim:field[@element='number'][not(@qualifier)]">
			
                    <xsl:for-each select="dim:field[@element='number'][not(@qualifier)]">
                            <xsl:text>(</xsl:text>
                                <xsl:value-of select="./node()"/>
								<xsl:text>)</xsl:text>
                                <xsl:if test="count(following-sibling::dim:field[@element='number'][not(@qualifier)]) != 0">
                            </xsl:if>
...
<!--START format - strany - v kapitole rozsah stran-->
		<xsl:choose>
            <xsl:when test="dim:field[@element='format'][not(@qualifier)]">
                    ...
                    <xsl:for-each select="dim:field[@element='format'][not(@qualifier)]">
                            <xsl:text>, </xsl:text><!--za carku se v pripade zmeny formy uvede s., jako strany-->
                                <xsl:value-of select="./node()"/>
								<xsl:text>. </xsl:text>
                                <xsl:if test="count(following-sibling::dim:field[@element='format'][not(@qualifier)]) != 0">
                            </xsl:if>
...
<!--START doi-->
	...
	</xsl:if><!--STOP Citace pro formu J_ČLÁNEK-->
 

<xsl:if test="contains(., 'B_KNIHA - CELEK')"><!--START Citace pro formu B_KNIHA - CELEK-->
...
<!--START doi-->
	...
	</xsl:if><!--STOP Citace pro formu B_KNIHA - CELEK-->
	
	
	
<!--START Citace pro formu Nmets-Metodiky schválené příslušným orgánem-->
	<xsl:if test="contains(., 'Nmets-Metodiky schválené příslušným orgánem')">
...
<!--START Vydavatel-->
        ...
        </xsl:choose>Certifikovaná metodika.<!--STOP Vydavatel-->
...
<!--START doi-->
	...
	</xsl:if><!--STOP Citace pro formu Nmets-Metodiky schválené příslušným orgánem-->
	
	<!--START Citace pro formu Nmetc-Metodiky certifikované oprávněným orgánem-->
	<xsl:if test="contains(., 'Nmetc-Metodiky certifikované oprávněným orgánem')">
...
<!--START doi-->
	...
	</xsl:if><!--STOP Citace pro formu Nmetc-Metodiky certifikované oprávněným orgánem-->
	
	
	
	
	<!--START Citace pro formu D_Stať ve sborníku-->
	<xsl:if test="contains(., 'D_Stať ve sborníku')">
...
<!--START doi-->
	...
	</xsl:if><!--STOP Citace pro formu D_Stať ve sborníku-->

<!--START Citace pro formu Other-->
...
<!--title idealne jen v jedné jazykove forme, problem u cs publikaci s nezadoucim en nazvem-->
		<xsl:choose>
            ...
            <xsl:when test="count(dim:field[@element='title'][not(@qualifier)]) = 1">
                   ...
                   <span style="font-style: italic"><xsl:value-of select="dim:field[@element='title'][not(@qualifier)][1]/node()"/>. </span>
...
<!--START doi-->
	...
	</xsl:if><!--STOP Citace pro formu Other-->

		
		<h5>Tyto citace vytvořil software a mohou obsahovat chyby. Uvedená citace není v souladu s normou ČSN ISO 690:2011.</h5>
    </xsl:template>
<!--KONEC VELKE CITACE-->
